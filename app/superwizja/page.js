...
async function getMainSupervisionArticle() {
  const resArticles = await client.getEntries({
    content_type: "supervisionArticle",
  });

  return resArticles.items.find((article) => article.fields.isMain);
}

export async function generateMetadata() {
  const mainArticle = await getMainSupervisionArticle();

  return {
    title: `Superwizja | ${mainArticle.fields.title}`,
    description:
      mainArticle.fields.lead ||
      "Informacje o superwizji pracy socjalnej w ramach działalności Polskiego Towarzystwa Superwizji Pracy Socjalnej",
    keywords: [
      "superwizja",
      "praca socjalna",
      "wsparcie",
    ...
    openGraph: {
      title: `Superwizja | ${mainArticle.fields.title}`,
      description:
        mainArticle.fields.lead || "Informacje o superwizji pracy socjalnej",
      type: "website",
    },
  };
}
    ...
    <div>
      ...
      <section className="grid grid-cols-1 gap-1 lg:grid-cols-3 lg:gap-8 mb-10">
        <div className="col-span-2">
          <SingleArticle title={mainArticle.fields.title} className="py-0">
